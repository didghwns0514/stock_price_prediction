...
class MyWindow(QMainWindow, form_class):    # MyWindow 클래스 QMainWindow, form_class 클래스를 상속 받아 생성됨
    def __init__(self): # MyWindow 클래스의 초기화 함수(생성자)
        ...
        self.kiwoom.OnReceiveTrCondition.connect(self.receive_tr_condition)  # 키움 사용자 조건검색식 초기 조회 시 반환되는 값이 있을 경우 receive_tr_condition 함수 호출
        self.kiwoom.OnReceiveRealCondition.connect(self.receive_real_condition)  # 키움 사용자 조건검색식 실시간 조회 반환되는 값이 있을 경우 receive_real_condition 함수 호출
    ...
    def receive_tr_condition(self, sScrNo, strCodeList, strConditionName, nIndex, nNext):   # 조건검색 초기 조회시 반환되는 값을 받는 함수
        logger.info("receive_tr_condition sScrNo: " + str(sScrNo) + ", strCodeList: " + str(strCodeList) + ", strConditionName: " + str(strConditionName) + ", nIndex: " + str(nIndex) + ", nNext: " + str(nNext))  # info 레벨 로그를 남김
 
    # strCode : 종목코드
    # strType : 이벤트 종류, "I":종목편입, "D", 종목이탈
    # strConditionName : 조건식 이름
    # strConditionIndex : 조건명 인덱스
    def receive_real_condition(self, strCode, strType, strConditionName, strConditionIndex):    # 조건검색 실시간 조회시 반환되는 값을 받는 함수
        logger.info("receive_real_condition strCode: " + str(strCode) + ", strType: " + str(strType) + ", strConditionName: " + str(strConditionName) + ", strConditionIndex: " + str(strConditionIndex))   # info 레벨 로그를 남김
        ...
        if str(strType) == "I":  # 편입 종목이라면
            self.pteLog.appendPlainText(str(logDateTime) + " 편입 신호 : " + str(strCode) + ", " + str(strCodeName)) # 트레이딩 화면 내역에 로그를 남김
        elif str(strType) == "D":   # 이탈 종목이라면
            self.pteLog.appendPlainText(str(logDateTime) + " 이탈 신호 : " + str(strCode) + ", " + str(strCodeName)) # 트레이딩 화면 내역에 로그를 남김
    ...
    def receive_trdata(self, screen_no, rqname, trcode, recordname, prev_next, data_len, err_code, msg1, msg2): # 키움 데이터 수신 함수
        if rqname == "opw00001_req":    # 수신된 데이터 구분명이 opw00001_req 일 경우 (예수금상세현황)
            ...
            ret = self.kiwoom.dynamicCall("SendCondition(QString,QString, int, int)", "0156", conditionName, conditionIndex, 1) # 선택한 조건검색을 화면번호 0156 실시간 조건검색 타입으로 호출
